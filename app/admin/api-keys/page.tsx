...
export default async function AdminApiKeysPage() {
  ...
  // Process keys to get last 4 characters
  const apiKeys: ApiKeyData[] = apiKeysRaw.map((key) => {
    let lastFourChars = "****";
    try {
      const decryptedKey = decryptKey(key.key_encrypted);
      lastFourChars = decryptedKey.slice(-4);
    } catch {
      // Keep masked value on decryption error
    }
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white px-4 py-12 dark:from-gray-900 dark:to-gray-950">
      <div className="mx-auto max-w-4xl">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            Aggiungi, gestisci e monitora le chiavi API OpenRouter
          </p>
        </div>

        {/* API Key List with management functionality */}
